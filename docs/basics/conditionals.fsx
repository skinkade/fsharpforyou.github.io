...
and execute a conditional branch of code if that condition yields a ``true`` value: ``if X do Y, else do Z``
*)
...
If the first branch returns a ``unit`` value, the second branch cannot return a ``string`` value as ``string`` is not convertible to ``unit``.
